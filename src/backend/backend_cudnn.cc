...
static inline void dump_dev_prop(cudaDeviceProp * prop, int dev_id){
    std::cout<<"Device " << dev_id << ": " << prop->name<<std::endl;
    std::cout<<"\tArch:\t" << prop->major<<prop->minor<<std::endl;  // cuda compute capability
    std::cout<<"\tGMem:\t" << prop->totalGlobalMem/1024/1024 << " MiB"<<std::endl;
    std::cout<<"\twarps:\t" << prop->warpSize<<std::endl;
    std::cout<<"\tCUs:\t" << prop->multiProcessorCount<<std::endl;
    std::cout<<"\tMaxClk:\t" << prop->clockRate<<std::endl;
    std::cout<<"\tMemClk:\t" << prop->memoryClockRate<<std::endl;
}
